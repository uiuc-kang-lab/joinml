...
class Process(object):
    def __init__(self, measure, rtable: List, lock: multiprocessing.Lock, output_filename: str) -> None:
    ...
    def __call__(self, args):
        ...
        for id2, tokens2 in self.rtable:
            results.append([id1, id2, self.measure.score_function(tokens1, tokens2)])
...
def run_lexical_similarity_proxy(ltable: List[int|str], rtable: List[int|str], output_filename: str, method: str="cosine", num_worker: int="1"):
    ...
    with multiprocessing.Pool(processes=num_worker) as pool:
        pool.map(Process(measure, rtable_tokenized, lock, output_filename), ltable_tokenized)
