...
def run_once(config: Config, dataset: ScalableJoinDataset, count_gt, strata_sample_sizes):
    ...
    for stratum_id in range(dataset.K+1):
        ...
        stratum_count_mean = np.mean(stratum_sample_count_results)
    ...
    if sum(strata_sample_count_results[0]) == 0:
        ...
        sampling_strata, blocking_strata = adaptive_allocation(strata_population_size,
                                                               strata_count_mean_vars, strata_count_mean)
    ...
    if np.sum(extra_sample_size) > 0:
        for strata_id, extra_size in zip(sampling_strata, extra_sample_size):
            ...
            strata_sample_count_results[strata_id] = np.concatenate([strata_sample_count_results[strata_id], extra_stratum_sample_count_results])
    ...
    if len(blocking_strata) > 0:
        for i in blocking_strata:
            ...
            for blocking_s_id in blocking_sample_ids:
                if dataset.run_oracle(blocking_s_id):
                    strata_sample_count_results[i].append(1)
                else:
    ...
    for stratum_id, stratum_sample_count_result in enumerate(strata_sample_count_results):
        count_estimation += np.mean(stratum_sample_count_result) * dataset.get_stratum_size(stratum_id)
