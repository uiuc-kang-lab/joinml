...
def run_once(config, dataset, oracle, dataset_sizes, max_statistics, min_statistics,
    ...
    if config.task == "joinml-ci" and sum(strata_sample_results[0]) == 0:
        # sampling with replacement on the blocking region
        blocking_region = np.concatenate(strata_population[1:])
        blocking_region_sample = np.random.choice(blocking_region, len(blocking_region), replace=True)
        blocking_region_sample_ids = np.array(np.unravel_index(blocking_region_sample, dataset_sizes)).T
        blocking_region_sample_results = []
        blocking_region_sample_count_results = []
        blocking_region_sample_avg_results = []
        for blocking_region_s_id, blocking_region_s in zip(blocking_region_sample_ids, blocking_region_sample):
            if oracle.query(blocking_region_s_id):
                stats = dataset.get_statistics(blocking_region_s_id)
                blocking_region_sample_results.append(stats)
                blocking_region_sample_count_results.append(1)
                blocking_region_sample_avg_results.append(stats)
            else:
                blocking_region_sample_results.append(0)
                blocking_region_sample_count_results.append(0)
        blocking_region_sample_results = np.array(blocking_region_sample_results)
        blocking_region_sample_count_results = np.array(blocking_region_sample_count_results)
        blocking_region_sample_avg_results = np.array(blocking_region_sample_avg_results)

        count_result = np.sum(blocking_region_sample_count_results)
        sum_result = np.sum(blocking_region_sample_results)
        avg_result = np.mean(blocking_region_sample_avg_results).item()

        # get CI by bootstrapping
        bootstrapping_counts = []
        bootstrapping_sums = []
        bootstrapping_avgs = []
        for _ in range(config.bootstrap_trials):
            resample = np.random.choice(blocking_region_sample_results, len(blocking_region_sample_results), replace=True)
            count_resample = np.random.choice(blocking_region_sample_count_results, len(blocking_region_sample_count_results), replace=True)
            avg_resample = np.random.choice(blocking_region_sample_avg_results, len(blocking_region_sample_avg_results), replace=True)

            resample_sum_results = np.sum(resample)
            resample_count_results = np.sum(count_resample)
            resample_avg_results = np.mean(avg_resample)

            bootstrapping_counts.append(resample_count_results)
            bootstrapping_sums.append(resample_sum_results)
            bootstrapping_avgs.append(resample_avg_results)
        
        count_lower, count_upper = get_ci_bootstrap(bootstrapping_counts, confidence_level=config.confidence_level)
        sum_lower, sum_upper = get_ci_bootstrap(bootstrapping_sums, confidence_level=config.confidence_level)
        avg_lower, avg_upper = get_ci_bootstrap(bootstrapping_avgs, confidence_level=config.confidence_level)

    else:
        # calculate the mean
        strata_population_size = [len(stratum_population) for stratum_population in strata_population]
        count_mean, sum_mean, avg_mean, blocking_result = \
            get_means(config, strata, strata_sample_sizes, 
        ...
        if config.need_ci:
            if sum(strata_sample_results[0]) > 0:
                ...
                count_lower, count_upper, sum_lower, sum_upper = \
                    get_non_positive_ci(max_statistics, config.confidence_level, 
                                        strata_population_size[0], strata_sample_sizes[0])
                blocking_count, blocking_sum, blocking_avg = blocking_result
                assert blocking_count is not None and blocking_sum is not None and blocking_avg is not None

                # get ci with two portions
                avg_upper = (max_statistics * count_upper + blocking_sum) / (count_upper + blocking_count)
                avg_lower = (min_statistics * count_upper + blocking_sum) / (count_upper + blocking_count)
                count_lower += blocking_count
                count_upper += blocking_count
                sum_lower += blocking_sum
                sum_upper += blocking_sum
        else:
...
def get_bootstrapping_ci(config, strata, strata_sample_sizes, strata_sample_results, strata_sample_count_results, strata_sample_avg_results, strata_gt, strata_count_gt, strata_population_size):
    ...
    for _ in range(config.bootstrap_trials):
        ...
        count_mean, sum_mean, avg_mean, _ = get_means(config, strata, strata_sample_sizes, 
...
def get_means(config, strata, strata_sample_sizes, strata_means, strata_mean_vars, strata_count_means, strata_count_mean_vars, strata_avg_means, strata_avg_mean_vars, strata_gt, strata_count_gt, strata_population_size):
    if np.isnan(strata_avg_means[0]):
        ...
        blocking_result = (count_mean * blocking_population_size, sum_mean * blocking_population_size, avg_mean)
        ...
        blocking_result = (None, None, None)
    
    return count_mean,sum_mean,avg_mean,blocking_result
