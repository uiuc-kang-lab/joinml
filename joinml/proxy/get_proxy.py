...
def get_proxy_score(config: Config, dataset: JoinDataset):
    # check cache for proxy scores
    proxy_store_path = f"{config.cache_path}/{config.dataset_name}_{config.proxy.split('/')[-1]}_scores.npy"
    os.system(f"touch {config.cache_path}/{config.dataset_name}_{config.proxy.split('/')[-1]}_test.npy")
...
def get_proxy_rank(config: Config, dataset: JoinDataset, proxy_scores: np.ndarray=None):
    dataset_sizes = dataset.get_sizes()
